...
class AIReportService:
    ...
    def generate_report_docx(
        ...
        # 🔥 Enrich action catalog recommendations (parallel + cache)
        for domain in report_json.get("domains", []):
            domain["actions"] = self._enrich_action_catalog_recommendations_parallel(
                project_id=project_id,
                scope=scope,
                user_id=user_id,
                action_rows=domain.get("actions", []),
    ...
    def _build_report_model(
        ...
        # Global score
        # ----------------------------
        avgs = [float(v.get("avg", 0.0)) for v in (domain_metrics or {}).values()]
        ...
        for item in (dep_json.get("inconsistencies") or []):
            if isinstance(item, dict):
                dep_index[
                    (str(item.get("domain_acronym")), str(item.get("reference_acronym")))
                ] = item

        domains_out: List[Dict[str, Any]] = []

        # ----------------------------
        # Loop domains in flow order
        # ----------------------------
        for d in (domain_flow or []):
            ac = str(d.get("acronym") or "").strip()
            if not ac or ac not in (domain_metrics or {}):
                continue

            m = domain_metrics[ac]
            domain_id = int(m.get("domain_id"))
            domain_key = self._domain_id_to_key(domain_id)

            qmap: Dict[str, int] = answers_map.get(domain_key) or {}
            if not qmap:
                continue

            theory_entries = ((domain_theory.get(ac) or {}).get("decision_tree") or [])
            action_catalog_entries = ((domain_theory.get(ac) or {}).get("action_catalog") or [])            
                                
            
            # ---- index decision_tree by (Question, Score)

            dt_by_qs: Dict[Tuple[str, int], Dict[str, Any]] = {}

            for row in theory_entries:
            ...
            # ----------------------------
            # Collect questions + resolve actions
            # ----------------------------
            questions_out: List[Dict[str, Any]] = []
            domain_scores: List[int] = []
            triggered_action_codes: Set[str] = set()
            ...
            for qid, score in qmap.items():
                ...
                domain_scores.append(score_int)

                dt_row = dt_by_qs.get((str(qid).strip(), score_int))
                ...
                if dt_row:
                    q_entry["text"] = dt_row.get("Text")
                    q_entry["objective"] = dt_row.get("Objective")
                    q_entry["cross_reference"] = dt_row.get("Cross_Reference")

                    acode = dt_row.get("Action_Code")
                    if acode:
                        triggered_action_codes.add(str(acode))
            ...
            avg = round(sum(domain_scores) / len(domain_scores), 2)
            floor_level = int(avg)
            
            # ----------------------------
            # Resolve action_catalog (AGGREGATED CORRECT)
            # ----------------------------

            normalized_triggered = {
                str(x).strip().upper()
                for x in triggered_action_codes
            }

            actions_raw = [
                row for row in action_catalog_entries
                if str(row.get("Action_Code") or "").strip().upper()
                in normalized_triggered
            ]
            ...
            actions_grouped: List[Dict[str, Any]] = []
            
            print("TRIGGERED SET RAW:", triggered_action_codes)
            print("TRIGGERED NORMALIZED:", [repr(x) for x in triggered_action_codes])
            print("CATALOG NORMALIZED:", [repr(str(r.get("Action_Code"))) for r in action_catalog_entries])
            
            print("ACTIONS_RAW SIZE:", len(actions_raw))
            print("ACTIONS_RAW CODES:", [r.get("Action_Code") for r in actions_raw])
            ...
            for code, rows in grouped.items():
                ...
                for r in rows:
                    ...
                    if r.get("Procedure"):
                        procedures.append(r.get("Procedure"))
                    ...
                    if r.get("Deliverables"):
                        dls = r.get("Deliverables")
                        if isinstance(dls, list):
                            deliverables.extend(dls)
                        else:
                            deliverables.append(dls)

                actions_grouped.append({
                    "Action_Code": code,
                    "Text": text_val,
            ...
            # ---- recommended_action via floor rule
            main_code = None

            # tenta pegar action do nível floor via Q1
            dt_main = dt_by_qs.get(("Q1", floor_level))
            if dt_main:
                main_code = dt_main.get("Action_Code")

            if not main_code and actions_grouped:
                main_code = actions_grouped[0]["Action_Code"]

            recommended_action = next(
                (a for a in actions_grouped if a["Action_Code"] == main_code),
                {
                    "Action_Code": None,
                    "Text": None,
                    "Procedures": [],
            ...
            # Cross references
            # ----------------------------
            refs = extract_domain_cross_references(questions_out)
            ...
            for (p_ac, r_ac), dep in dep_index.items():
                if p_ac == ac:
            ...
            domains_out.append({
                ...
                "name": d.get("name"),
                "average_score": avg,
                "level_floor": floor_level,
                "level_target": floor_level + 1,
                "questions": questions_out,
                "actions": actions_grouped,
    ...
    def _enrich_action_catalog_recommendations_parallel(
        self,
        project_id: str,
        scope: str,
        user_id: Optional[str],
        ...
        """
        Enrich Recommendations based on action_catalog.Text.
        Parallel execution + cache per recommendation.
        """
        ...
        cache_root = _cache_dir(self.base_dir, project_id, scope, user_id)
        enrich_cache_path = cache_root / "action_recommendation_enrichment.json"
        ...
        def _fingerprint(row, recommendation):
            base = {
                "model_hash": model_hash,
        ...
        def _worker(row, recommendation):
            fp = _fingerprint(row, recommendation)

            if not force_regen and fp in enrich_cache:
                return row["Hash_Key"], recommendation, enrich_cache[fp]

            system_msg = (
                "Return only professional text. "
                "Maximum 3 compact paragraphs. "
    ...
    Instruction:
    Expand and professionalize the recommendation.
    Base strictly on the procedure text.
    Clarify what must be done, why, and expected outcome.
    Do not repeat the procedure text.
    Produce up to 3 compact paragraphs.
    """.strip()

            try:
                enriched = self.ai_call([
                    {"role": "system", "content": system_msg},
                    {"role": "user", "content": user_msg},
                ])
                enriched = enriched.strip()
            except Exception:
                enriched = recommendation  # fallback

            enrich_cache[fp] = enriched
            return row["Hash_Key"], recommendation, enriched

        # Collect tasks
        for row in action_rows:
            for rec in row.get("Recommendations", []):
                tasks.append((row, rec))
        ...
        with ThreadPoolExecutor(max_workers=self.ai_workers) as executor:
            futures = [
                executor.submit(_worker, row, rec)
                for row, rec in tasks
            ]

            for future in as_completed(futures):
                hash_key, original_rec, enriched_text = future.result()
                results_map.setdefault(hash_key, {})[original_rec] = enriched_text

        # Apply enriched results
        for row in action_rows:
            hash_key = row.get("Hash_Key")
            if hash_key in results_map:
                row["Recommendations"] = [
                    results_map[hash_key].get(rec, rec)
                    for rec in row.get("Recommendations", [])
                ]
    ...
    def _report_json_cache_path(self, project_id: str, scope: str, user_id: Optional[str], model_hash: str) -> Path:
        mh = (model_hash or "")[:16]  # corta hash para evitar Windows path limit
        return _cache_dir(self.base_dir, project_id, scope, user_id) / f"rje_{mh}.json"

    def _translation_cache_path(self, project_id: str, scope: str, user_id: Optional[str], model_hash: str, target_language: str) -> Path:
        mh = (model_hash or "")[:16]
        lang = (target_language or "xx")[:5]
        return _cache_dir(self.base_dir, project_id, scope, user_id) / f"rjt_{lang}_{mh}.json"
