...
class AIReportService:
    ...
    def _add_blueprint_section(
            ...
            # -------------------------------------------------
            # 3.1 Conteúdo global do artefato (não domain-specific)
            # -------------------------------------------------
            self._add_heading(doc, "3.1 Blueprint Overview", level=2)
            ...
            def is_global_section(title: str) -> bool:
                ...
                return t in global_section_titles

            # Renderiza as seções globais uma vez, pegando do DEMO do 1º domínio
            if scores:
                s0 = scores[0]
                ...
                for sec in global_sections0:
                    section_number = f"3.1.{sec_counter}"
                    sec_counter += 1

                    self._add_heading(doc, f"{section_number} {sec['title']}", level=3)
            ...
            self._add_heading(doc, "3.2 Maturity Model Definition", level=2)
            ...
            if isinstance(maturity_tree, dict) and maturity_tree.get("maturity_scale"):
                ...
                self._add_paragraph(doc, T("no_maturity_scale") if self._has_text("no_maturity_scale") else "")

            # -------------------------------------------------
            # 3.3+ Domínios (domain specific)
            # -------------------------------------------------
            for idx, s in enumerate(scores, start=1):

                chapter_number = f"3.{idx + 2}"
                ...
                # domain-specific: remove seções globais para evitar repetição
                sections = [sec for sec in sections if not is_global_section(sec.get("title", ""))]

                render_example_block = True

                # carregar decision_tree e action_catalog do projeto
                ...
                for sec_index, sec in enumerate(sections, start=1):
                    ...
                    for block in sec["blocks"]:
                        ...
                        if block["kind"] == "subsection":
                            ...
                            # 4a alteração: procedures só 1 vez por domínio (Q1 como referência)
                            sec_title = str(sec.get("title") or "").strip()
                            is_procedure_definition_local = ("Definição" in sec_title) and ("Procedimento" in sec_title)

                            if (
                                render_example_block
                                and is_procedure_definition_local
                            ...
                            # override "Definição do Domínio" -> "DG - Governança de Dados"
